...
\lstinputlisting[language=Matlab, caption={\texttt{Projectile} function described in (\ref{eq:ode45}).},label={lst:projectile}]{Projectile.m}

Furthermore, we define the events function discussed in Observation III as follows:\\

\lstinputlisting[language=Matlab, caption={\texttt{Events1} function described in Observation III.}, label={lst:events}]{Events1.m}
...
\lstinputlisting[language=Matlab, caption={\texttt{Distance} function to get the distance travelled for given angle.}, label={lst:distance}]{Distance.m}
...
\lstinputlisting[language=Matlab, caption={\texttt{FindMaxDistTheta} to find the optimal distance and the angle.}, label={lst:findmaxdist}]{FindMaxDistTheta.m}
...
Therefore, we can conclude that a maximum distance of 19617.77m (2dp) is achieved with a firing angle of $44.60^{\circ}$ or equivalently 0.77842 radians. %Let us plot this projectile motion below.

%\begin{figure}[h]
%	\centering
%  	\includegraphics[scale=0.5]{assets/max_dist_plot.eps}
%  	\caption{\texttt{Distance(0.77842,true)}: projectile motion for maximum distance.}
%  	\label{fig:plot}
%\end{figure}
...
%\lstinputlisting[language=Matlab]{Distance.m}
...
\lstinputlisting[language=Matlab, caption={\texttt{FindLaunchAngles(target)} returns the two launch angles that hit the \texttt{target}.},label={lst:launch_angle}]{FindLaunchAngles.m}
\ \\
...
\lstinputlisting[language=Matlab, caption={\texttt{FindLaunchAngles(target)} returns the two launch angles that hit the \texttt{target}.},label={lst:heightduration}]{GetHeightAndDurationAt12K.m}
...
\lstinputlisting[language=Matlab, caption={\texttt{Events2} function.},label={lst:events2}]{Events2.m}
\ \\
...
\lstinputlisting[language=Matlab, caption={\texttt{TimeDelay(angles)} outputs one time delay $t_d$ for the given \texttt{angles}.},label={lst:timedelay}]{TimeDelay.m}
